...
def text2html(text):
    lines = map(lambda s: '<p>%s</p>' % s.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;'), filter(lambda s: s.strip() != '', text.spilt('/n')))
    return ''.join(lines)
...
def index(request):
    ...
    blogs = [
        ...
        Blog(id='3', name='Python', summary=summary,
             created_at=time.time() - 4800)
    ]
    return {
...
@post('/api/users')
async def api_register_uer(*, email, name, passwd):
